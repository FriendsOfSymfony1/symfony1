public function getListParams()
  {
    return <?php echo $this->asPhp($this->config['list']['params'] ?? '%%'.implode('%% - %%', $this->config['list']['display'] ?? $this->getAllFieldNames(false)).'%%') ?>;
<?php unset($this->config['list']['params']) ?>
  }

  public function getListLayout()
  {
    return '<?php echo $this->config['list']['layout'] ?? 'tabular' ?>';
<?php unset($this->config['list']['layout']) ?>
  }

  public function getListTitle()
  {
    return '<?php echo $this->escapeString($this->config['list']['title'] ?? sfInflector::humanize($this->getModuleName()).' List') ?>';
<?php unset($this->config['list']['title']) ?>
  }

  public function getEditTitle()
  {
    return '<?php echo $this->escapeString($this->config['edit']['title'] ?? 'Edit '.sfInflector::humanize($this->getModuleName())) ?>';
<?php unset($this->config['edit']['title']) ?>
  }

  public function getNewTitle()
  {
    return '<?php echo $this->escapeString($this->config['new']['title'] ?? 'New '.sfInflector::humanize($this->getModuleName())) ?>';
