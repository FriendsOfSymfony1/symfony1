...
$mailer = new sfMailer($dispatcher, [
    ...
    'transport' => ['class' => 'TestMailerTransport', 'param' => ['foo' => 'bar', 'bar' => 'foo']],
]);
...
$mailer = new sfMailer($dispatcher, [
    ...
    'spool_arguments' => ['TestMailMessage'],
    'transport' => ['class' => 'Swift_SmtpTransport', 'param' => ['username' => 'foo']],
]);
$t->is($mailer->getRealTimeTransport()->getUsername(), 'foo', '__construct() passes the parameters to the main transport');

try {
    $mailer = new sfMailer($dispatcher, ['delivery_strategy' => 'spool']);
...
$mailer = new sfMailer($dispatcher, ['delivery_strategy' => 'spool', 'spool_class' => 'TestSpool']);
...
// single address
try {
    $mailer = new sfMailer($dispatcher, ['delivery_strategy' => 'single_address']);
...
// ->compose()
$t->diag('->compose()');
$mailer = new sfMailer($dispatcher, ['delivery_strategy' => 'none']);
...
$t->is($message->getFrom(), ['from@example.com' => ''], '->compose() takes the from address as its first argument');
$t->is($message->getTo(), ['to@example.com' => ''], '->compose() takes the to address as its second argument');
...
$mailer = new sfMailer($dispatcher, ['logging' => true, 'delivery_strategy' => 'none']);
...
$t->is($messages[0]->getFrom(), ['from@example.com' => ''], '->composeAndSend() takes the from address as its first argument');
$t->is($messages[0]->getTo(), ['to@example.com' => ''], '->composeAndSend() takes the to address as its second argument');
...
$mailer = new sfMailer($dispatcher, [
    ...
    'spool_class' => 'TestSpool',
    'spool_arguments' => ['TestMailMessage'],
    'transport' => ['class' => 'TestMailerTransport'],
]);
