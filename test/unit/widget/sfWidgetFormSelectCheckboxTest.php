...
// ->render()
$t->diag('->render()');
$w = new \sfWidgetFormSelectCheckbox(['choices' => ['foo' => 'bar', 'foobar' => 'foo'], 'separator' => '']);
...
'</ul>';
$t->is($w->render('foo', ['foobar']), $output, '->render() renders a checkbox tag with the value checked');
...
'</ul>';
$t->is($w->render('foo', ['foobar'], ['onChange' => 'alert(42)']), $onChange, '->render() renders a checkbox tag using extra attributes');

$w = new \sfWidgetFormSelectCheckbox(['choices' => ['0' => 'bar', '1' => 'foo']]);
$output = <<< 'EOF'
...
$w = new \sfWidgetFormSelectCheckbox(['choices' => []]);
$t->is($w->render('myname', []), '', '->render() returns an empty HTML string if no choices');

// group support
$t->diag('group support');
$w = new \sfWidgetFormSelectCheckbox(['choices' => ['foo' => ['foo' => 'bar', 'bar' => 'foo'], 'bar' => ['foobar' => 'barfoo']]]);
$output = <<<'EOF'
...
EOF;
$t->is($w->render('foo', ['foo', 'foobar']), fix_linebreaks($output), '->render() has support for groups');

$w->setOption('choices', ['foo' => ['foo' => 'bar', 'bar' => 'foo']]);
$output = <<<'EOF'
...
EOF;
$t->is($w->render('foo', ['bar']), fix_linebreaks($output), '->render() accepts a single group');
...
$w = new \sfWidgetFormSelectCheckbox(['choices' => new \sfCallable('choice_callable')]);
...
$w = new \sfWidgetFormSelectCheckbox(['choices' => ['<b>Hello world</b>']]);
...
// __clone()
$t->diag('__clone()');
$w = new \sfWidgetFormSelectCheckbox(['choices' => new \sfCallable([$w, 'foo'])]);
$w1 = clone $w;
...
$w = new \sfWidgetFormSelectCheckbox(['choices' => new \sfCallable([$a = new \stdClass(), 'foo'])]);
$w1 = clone $w;
$callable = $w1->getOption('choices')->getCallable();
