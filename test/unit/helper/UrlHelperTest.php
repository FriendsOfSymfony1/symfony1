...
$t->is(link_to('test', '@homepage', ['absolute' => true]), '<a href="/module/action">test</a>', 'link_to() can take an "absolute" option');
$t->is(link_to('test', '@homepage', ['absolute' => false]), '<a href="module/action">test</a>', 'link_to() can take an "absolute" option');
$t->is(link_to('test', '@homepage', ['query_string' => 'foo=bar']), '<a href="module/action?foo=bar">test</a>', 'link_to() can take a "query_string" option');
$t->is(link_to('test', '@homepage', ['anchor' => 'bar']), '<a href="module/action#bar">test</a>', 'link_to() can take an "anchor" option');
...
$t->like(link_to('test', '@homepage', ['method' => 'post']), '/==TOKEN==/', 'link_to() includes CSRF token from BaseForm');
...
$t->is(button_to('test', '@homepage', ['query_string' => 'foo=bar']), '<input value="test" type="button" onclick="document.location.href=\'module/action?foo=bar\';" />', 'button_to() returns an HTML "input" tag');
$t->is(button_to('test', '@homepage', ['anchor' => 'bar']), '<input value="test" type="button" onclick="document.location.href=\'module/action#bar\';" />', 'button_to() returns an HTML "input" tag');
$t->is(button_to('test', '@homepage', ['popup' => 'true', 'query_string' => 'foo=bar']), '<input value="test" type="button" onclick="var w=window.open(\'module/action?foo=bar\');w.focus();return false;" />', 'button_to() returns an HTML "input" tag');
...
$t->like(button_to('test', '@homepage', ['method' => 'post']), '/==TOKEN==/', 'button_to() includes CSRF token from BaseForm');
...
$t->is(link_to_if(false, 'test', '@homepage', ['tag' => 'div']), '<div>test</div>', 'link_to_if() takes a "tag" option');
...
$t->is(link_to_if(true, 'test', '@homepage', ['tag' => 'div']), '<a href="module/action">test</a>', 'link_to_if() removes "tag" option (given as array) in true case');
$t->is(link_to_if(false, 'test', '@homepage', ['query_string' => 'foo=bar', 'absolute' => true, 'absolute_url' => 'http://www.google.com/']), '<span>test</span>', 'link_to_if() returns an HTML "span" tag by default if the condition is false');
$t->is(link_to_if(true, 'test', 'homepage', [], ['class' => 'test']), '<a class="test" href="homepage">test</a>', 'link_to_if() accepts link_to2 compatible usage');
$t->is(link_to_if(false, 'test', 'homepage', [], ['class' => 'test']), '<span class="test">test</span>', 'link_to_if() accepts link_to2 compatible usage');
...
$t->is(link_to_unless(true, 'test', 'homepage', [], ['class' => 'test']), '<span class="test">test</span>', 'link_to_unless() accepts link_to2 compatible usage');
$t->is(link_to_unless(false, 'test', 'homepage', [], ['class' => 'test']), '<a class="test" href="homepage">test</a>', 'link_to_unless() accepts link_to2 compatible usage');
...
preg_match('/href="(.+?)"/', mail_to('fabien.potencier@symfony-project.com', 'fabien', ['encode' => true]), $matches);
...
$t->isnt(mail_to('webmaster@example.com', 'encoded', ['encode' => true]), '<a href="mailto:webmaster@example.com">encoded</a>', 'mail_to with encoding works');

$t->is(mail_to('webmaster@example.com', '', [], ['subject' => 'test subject', 'body' => 'test body']), '<a href="mailto:webmaster@example.com?subject=test+subject&amp;body=test+body">webmaster@example.com</a>', 'mail_to() works with given default values in array form');
$t->is(mail_to('webmaster@example.com', '', [], 'subject=test subject body=test body'), '<a href="mailto:webmaster@example.com?subject=test+subject&amp;body=test+body">webmaster@example.com</a>', 'mail_to() works with given default values in string form');
$t->is(mail_to('webmaster@example.com', '', [], 'subject=Hello World and more'), '<a href="mailto:webmaster@example.com?subject=Hello+World+and+more">webmaster@example.com</a>', 'mail_to() works with given default value with spaces');
